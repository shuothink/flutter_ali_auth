...
extension AuthUIBuilder {
    ...
    func buildAlertModel(config: AuthUIConfig) -> TXCustomModel {
        ...
        model.exitAnimation = exitAnimationWithFadeOut
...
//            return CGRect(x: offsetX, y: offsetY, width: frame.width, height: frame.height)
        ...
        debugPrint("aliauth---\((config.checkBoxWH as Float?) ?? 0)")
        ...
        model.contentViewFrameBlock = {
            ...
            let offsetX: CGFloat = (superViewSize.width - frame.size.width) / 2
            
            let offsetY: CGFloat = (superViewSize.height - frame.size.height) / 2
            return CGRect(x: offsetX, y: offsetY, width: width, height: height)
        }
    ...
    var entryAnimation: CABasicAnimation {
    ...
    var exitAnimationWithFadeOut: CABasicAnimation {
    ...
    var exitAnimationWithScale: CABasicAnimation {
        let animation = CABasicAnimation(keyPath: "transform.scale")
        animation.fromValue = 1.0
